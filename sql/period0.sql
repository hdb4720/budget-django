...
FROM budget_periodscheme bs
    JOIN budget_period bp 
        ON bp.scheme_id = bs.id
    JOIN budget_transaction bt 
        ON bt.trn_date BETWEEN bp.start AND bp.end
    JOIN budget_category bc 
        ON bt.category_id = bc.id
    JOIN budget_account ba 
        ON bt.account_id = ba.id
WHERE LOWER(bs.name) ILIKE '%{1}%'
    AND bp.start <= to_date('{3}', 'MMDDYY')
    AND bp.end >= to_date('{2}', 'MMDDYY')
    AND (
           '{4}' = '%'
        OR '{4}' = '*'        
        OR '{4}' = '.'
        OR LOWER(bc.full_path) ILIKE '%{4}%'
        OR LOWER(ba.name) ILIKE '%{4}%'
        )
ORDER BY 
    bc.full_path,
    ba.short_name,
